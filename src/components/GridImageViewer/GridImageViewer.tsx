    ...
    >
      <VStack w={images.length > 1 ? "48%" : "100%"}>
        <Pressable
          ...
          onPress={() =>
            navigation.navigate(
              dashBoardRoutes.galleryScreen.path as never,
              {
      ...
      <VStack w="48%" h="100%" justifyContent={"space-between"}>
        <Stack
          direction={
            images.length === 2 || images.length === 3 ? "column" : "row"
          }
          w="100%"
        ...
        >
          {images[1] && (
            <Pressable
              width={
                images.length === 2 || images.length === 3 ? "100%" : "48%"
              }
              onPress={() =>
                navigation.navigate(
                  dashBoardRoutes.galleryScreen.path as never,
        ...
        <HStack w="100%" h="48%" space={2}>
          {images[3] && (
            <Pressable
              width={
                images.length === 3 || images.length === 4 ? "100%" : "48%"
              }
              onPress={() =>
                navigation.navigate(
                  dashBoardRoutes.galleryScreen.path as never,
          ...
          {images.length > 3 && (
            ...
            >
              <Box
                position="relative"
                bg={images.length > 5 ? "#000" : "transparent"}
                overflow={"hidden"}
                borderRadius={8}
                width="100%"
                h="100%"
              >
                <Image
                  style={{
                    ...
                    width: "100%",
                    borderRadius: 8,
                    opacity: images.length > 5 ? 30 : 100,
                  }}
                  source={{ uri: images[4] }}
                />
                {images.length >= 4 && (
